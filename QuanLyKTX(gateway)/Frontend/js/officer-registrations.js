// Officer Registrations Module
class OfficerRegistrations {
    constructor() {
        this.registrations = [];
        this.students = [];
        this.rooms = [];
    ...
    async bootstrap() {
        try {
            ...
            console.error('Failed to initialize OfficerRegistrations:', error);
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="7" class="text-center" style="color: #e74c3c;">
                            ...
                            <button class="btn btn-sm btn-primary" onclick="window.officerRegistrations.loadRegistrations()" style="margin-top: 10px;">
    ...
    async rejectRegistration() {
        ...
        try {
            ...
            const updateData = {
                ...
                LyDo: this.currentRegistration.lyDo || this.currentRegistration.LyDo || null,
                NgayDangKy: this.currentRegistration.ngayDangKy || this.currentRegistration.NgayDangKy,
                GhiChu: reason || 'Đã từ chối bởi nhân viên'
            };
...
function viewRegistrationDetails(registrationId) {
    window.officerRegistrations.viewDetails(registrationId);
}

function closeRegistrationModal() {
    window.officerRegistrations.closeModal();
}

function approveRegistration() {
    window.officerRegistrations.approveRegistration();
}
...
function rejectRegistration() {
    window.officerRegistrations.rejectRegistration();
}
...
function filterRegistrations() {
    ...
    let filtered = window.officerRegistrations.registrations.filter(reg => {
        const maDon = reg.maDon || reg.MaDon || '';
        const maSinhVien = reg.maSinhVien || reg.MaSinhVien;
        const trangThai = reg.trangThai || reg.TrangThai || '';
        ...
        if (reg.tenSinhVien || reg.TenSinhVien) {
            ...
            const student = window.officerRegistrations.students.find(s => (s.maSinhVien || s.MaSinhVien) === maSinhVien);
            if (student) {
    ...
    window.officerRegistrations.renderRegistrationsTable(filtered);
}

function resetFilters() {
    document.getElementById('searchInput').value = '';
    document.getElementById('statusFilter').value = '';
    window.officerRegistrations.renderRegistrationsTable();
}
...
document.addEventListener('DOMContentLoaded', () => {
    window.officerRegistrations = new OfficerRegistrations();
});
